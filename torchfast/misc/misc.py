...
def all_gather_same_ndim(tensor: torch.Tensor):
    # tensor must has same ndim on each node
    world_size = distributed.get_world_size()
    local_shape = torch.tensor(tensor.shape, device=tensor.device)
    shape_list = [torch.empty_like(local_shape, device=tensor.device) for _ in range(world_size)]
    distributed.all_gather(shape_list, local_shape)
    max_size = max([shape.prod() for shape in shape_list]).item()
    padded = torch.empty(max_size, device=tensor.device)
    padded[:local_shape.prod()] = tensor.reshape(-1)

    tensor_list = [torch.empty(max_size, device=tensor.device) for i in range(world_size)]
...
def seed_everything(seed: Optional[int] = None) -> int:
    ...
    Warning:
        ALWAYS SPECIFIY `seed` under distributed situation (eg. DDP training), 
        otherwise, each worker almost certainly will select a different seed.
    """
